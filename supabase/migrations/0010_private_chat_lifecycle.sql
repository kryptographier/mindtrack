...
--
-- A secret code is reusable by the FIRST user who redeems it
-- until its server-side expiry. It reopens that user's active
-- session instead of creating duplicate sessions.
-- Explicitly ending a session revokes its code.
-- Admin suspension is separate from code revocation.
-- =========================================================
...
alter table public.chat_sessions
  add column if not exists secret_code_id uuid references public.secret_codes(id);
...
alter table public.chat_sessions
  ...
  check (status in ('active', 'suspended', 'ended', 'expired'));

create index if not exists secret_codes_redeemed_by_idx
  on public.secret_codes(redeemed_by);

create index if not exists chat_sessions_secret_code_idx
  on public.chat_sessions(secret_code_id);

-- Existing migration 0008 was a persistent-code design, while
-- 0009 introduced expiry. Keep any already-issued valid codes
-- usable, but bind them to the first redeemer from this point on.
...
begin
  ...
  insert into public.secret_codes(
    code_hash, created_by, expires_at, max_attempts, attempt_count, used_at, revoked_at, redeemed_by
  ) values (
    v_hash, auth.uid(), v_expires_at, 999999, 0, null, null, null
...
begin
  ...
  -- The first successful redeemer owns the code for its lifetime.
  ...
  -- Reopen the user's current session if it is still genuinely active.
  ...
  if found then
    ...
    elsif now() - v_session.last_activity_at > (v_idle_minutes || ' minutes')::interval then
      update public.chat_sessions set status = 'expired' where id = v_session.id;
    else
  ...
  -- A suspended session is deliberately not reopened by the code.
  if exists (
...
-- ---------------------------------------------------------
-- Fix session validity to understand suspension and code expiry.
-- ---------------------------------------------------------

drop function if exists public.is_chat_session_valid(uuid);
...
-- ---------------------------------------------------------
-- Fix touch_chat_session so send_message rejects suspension/expiry.
-- ---------------------------------------------------------

drop function if exists public.touch_chat_session(uuid);
...
-- ---------------------------------------------------------
-- send_message() remains the canonical message RPC.
-- ---------------------------------------------------------

-- 0005 already defines send_message(). Recreate it so the current
-- lifecycle semantics are explicit and the frontend/RPC contract
-- cannot drift back to the old send_chat_message name.
drop function if exists public.send_chat_message(uuid, text);

-- ---------------------------------------------------------
-- End a session and revoke its code. This is deliberately different
-- from leaving the chat UI for the journal: navigation does not call
-- this function, so the same code can be used to return until expiry.
-- ---------------------------------------------------------
...
begin
  ...
  update public.chat_sessions
    ...
    where id = p_session_id and status in ('active', 'suspended');

  if v_row.secret_code_id is not null then
    update public.secret_codes
      set revoked_at = coalesce(revoked_at, now())
      where id = v_row.secret_code_id;
  end if;
...
-- ---------------------------------------------------------
-- Admin suspension/resume. Suspension does not revoke the code.
-- The admin can resume it while the code/session expiry remains valid.
-- ---------------------------------------------------------
...
begin
  ...
  update public.chat_sessions
    ...
    where id = p_session_id and status = 'active' and admin_id = auth.uid();
end;
$$;
...
-- Postgres Changes must have the message table in supabase_realtime
-- or inserts will never reach the browser subscribers.
do $$
begin
